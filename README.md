...
While a Codex turn is running, later queued chat messages are coalesced into the next turn when they belong to the same regular chat, or to the same `chatId + larkThreadId` in a topic chat. The batch preserves message order and per-message observability/reactions, and sends one combined reply against the newest message. Batches are capped at 20 messages and the configured input-character budget; overflow remains queued for a following turn. Document comments and messages from different chats or topics are never coalesced.
