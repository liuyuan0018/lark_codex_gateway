...
| 🤫 无需回复状态 | Agent 返回 `[NO_REPLY]` 时记录为正常结束状态，以区别于消息过滤或处理失败；外部应用发送的构建/运行失败卡片必须介入，不能静默忽略。 |
| 🔍 全流程观测 | 使用飞书 `eventId` 关联接收、排队、Codex 处理、授权、发送和失败记录。 |
| 🛡️ 重复与冲突保护 | 使用 `messageId` 合并 Bot 事件和用户轮询的同一条消息，对 Codex 活动写入冲突执行有界退避重试，并在持久化 session 引用失效时自动创建新任务并更新绑定。 |
...
当一个 Codex turn 正在处理时，同一普通群后续排队的消息会合并到下一个 turn；话题群则只合并相同 `chatId + larkThreadId` 的排队消息。批次保留每条消息的顺序、去重、状态表情和观测记录，但只运行一次 Codex，并把综合回复挂在最新一条消息下。单批最多 20 条且受配置的输入字符预算限制，超出的消息继续留在队列中。文档评论、不同群或不同话题的消息不会合并。

被网关接受的聊天消息会先由 Bot 添加 `OK` 表情，表示已经收到并进入队列；取得 Codex session 执行权后切换为 `Typing`。Codex 处理完成时状态表情会被移除，处理失败时则切换为 `ERROR`。重复、被过滤的消息和文档评论不使用这套状态表情。
